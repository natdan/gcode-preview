...
void main() {
	...
	int vertexNumber = int(a_color.r * 100.0) + int(a_color.g * 100.0) * 100 + int(a_color.b * 100.0) * 10000;
    int typ = int(a_color.a * 100.0);
    
    v_color = a_color;
    if (vertexNumber <= u_number) { // Before current movement
        if (typ == 0) { // Movement
            v_color = vec4(1.0, 1.0, 1.0, 0.3); // Faint gray
        } else if (typ == 1) { 
            v_color = vec4(0.3, 1.0, 0.3, 1.0); // Green - would be nice to parametrise it...
        } else {
            v_color = vec4(0.5, 0.5, 0.0, 1.0); // Dark yellow
        }
    } else { // After - not yet done
        if (typ == 0) { // Movement
            v_color = vec4(1.0, 1.0, 1.0, 0.0); // Not displayed
        } else if (typ == 1) {
            v_color = vec4(1.0, 0.0, 0.0, 0.1); // Faint red
        } else {
            v_color = vec4(1.0, 0.0, 1.0, 0.1); // Faint purple!!!
        }
    }
